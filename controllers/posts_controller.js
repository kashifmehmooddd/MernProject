...
const remove = async (req, res) => {
  try {
    ...
    if (post.user != req.user) return res.status(400).json({
...
const removeComment = async (req, res) => {
  try {
    ...
    if (!temp) return res.status(400).json({
      msg: 'Comment not found!'
    })

    if (temp.user != req.user) res.status(400).json({
...
const addLike = async (req, res) => {
  ...
  try {
    ...
    if (post.likes.some(l => l.user == req.user)) return res.status(400).json({
...
const removeLike = async (req, res) => {
  try {
    ...
    const temp = post.likes.find(l => l._id == req.params.like_id)
    ...
    post.likes = post.likes.filter(c => c._id != req.params.like_id)
